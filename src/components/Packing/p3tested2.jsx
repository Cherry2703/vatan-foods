...
export default function Packing() {
  ...
  const fetchBatchIds = async () => {
    try {
      ...
      setBatchList(
        (res.data || []).filter(b =>
          Number(b.remainingAfterCleaning) > 0 &&
          Number(b.outputQuantity) > 0
        )
      );

      console.log("all batch ID from cleaning to packing",res.data);
  ...
  //   const b = batchList.find(x => x.cleaningId === r.cleaningId);
  //   setSelectedBatch(b || null);
  //   const cleanedQty = b ? formatNumber(b.outputQuantity) : 0;
  //   setBatchCleanedQty(cleanedQty);
  ...
  if (b) {
    setSelectedBatch(b);
    setBatchCleanedQty(formatNumber(b.outputQuantity));
    setRemainingQty(formatNumber(b.outputQuantity) - formatNumber(r.outputPacked));
  }

const inc = incomingList.find(
  it => it.batchId === r.batchId && it.cleaningId === r.cleaningId
);
  setSelectedIncoming(inc || null);
...
<select
  ...
  onChange={(e) => {
    ...
    setFormData(prev => ({
      ...
      inputFromCleaning: formatNumber(batch.outputQuantity),
      // ❗ DO NOT force outputPacked here
      outputPacked: prev.outputPacked || 0
    }));

    setSelectedBatch(batch);
    setBatchCleanedQty(formatNumber(batch.outputQuantity));
    setRemainingQty(formatNumber(batch.outputQuantity));
  }}
>
...
{batchList
  .filter(b => b.cleaningId)
  .map((b, index) => (
    <option key={`${b.cleaningId}-${index}`} value={b.cleaningId}>
      {b.cleaningId} || {b.batchId} || {b.itemName} || Cleaned: {b.outputQuantity} kg
    </option>
))}
